...
/**
 * This class represents command to sort text's lexemes by number
 * of entered symbol in them.
 * @author Pavel Isidovich
 *
 */
public class SortTextLexemesCommand implements Command {
	...
	public void execute() {
		...
		TextOperation to = sf.getSortTextLexemesService();

		try {
			String[] request = mw.makeRequest(rb.getString("pathRequest2"))
																.split(" ");
			if (request[1].length() != 1) {
				...
				mw.showInfo(to.sortByEnteredSymbol(request[0], request[1]));
			}
